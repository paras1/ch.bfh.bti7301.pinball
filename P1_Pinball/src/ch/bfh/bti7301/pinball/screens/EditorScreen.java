	...
	public void show() {
		...
		slNumBalls.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				lbNumBalls.setText("Number of balls:   "
						+ (int) slNumBalls.getValue());
			}
		});
		...
		slNumBumpers.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				lbNumBumpers.setText("Number of bumpers:   "
						+ (int) slNumBumpers.getValue());
			}
		});
		...
		slLauncherSpeed.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				lbLauncherSpeed.setText("Launcher speed:   "
						+ (int) slLauncherSpeed.getValue());
			}
		});
		...
		slFlipperSpeed.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				lbFlipperSpeed.setText("Flipper speed:   "
						+ (int) slFlipperSpeed.getValue());
			}
		});
		...
		slSlingshot.addListener(new ChangeListener() {
			public void changed(ChangeEvent event, Actor actor) {
				lbSlingshot.setText("Slingshot speed:   "
						+ (int) slSlingshot.getValue());
			}
		});
		...
		btCreateBoard.addListener(new InputListener() {
			public boolean touchDown(InputEvent event, float x, float y,
				...
				EditorBuffer.getInstance().setNumBumpers(
						(int) slNumBumpers.getValue());
				EditorBuffer.getInstance().setActualBumperNum(1);
				EditorBuffer.getInstance().setNumBalls(
						(int) slNumBalls.getValue());
				EditorBuffer.getInstance().setLauncherSpeed(
						(int) slLauncherSpeed.getValue());
				EditorBuffer.getInstance().setMpNewBoardBuffer(
						FieldLayout.readFieldLayout(level, true));
				EditorBuffer.getInstance().putMpNewBoardBufferElement(
						"numballs", (int) slNumBalls.getValue());
				EditorBuffer.getInstance().setFlipperSpeed(
						(int) slFlipperSpeed.getValue());
				EditorBuffer.getInstance().setSlingshotSpeed(
						(int) slSlingshot.getValue());

				ArrayList<HashMap> alFlipper = EditorBuffer.getInstance()
						.getAlFlipperBuffer();
				for (HashMap mpFlipper : alFlipper) {
					mpFlipper.put("upspeed", EditorBuffer.getInstance()
							.getFlipperSpeed());
					mpFlipper.put("downspeed", EditorBuffer.getInstance()
							.getFlipperSpeed());
				}

				ArrayList<HashMap> alElements = EditorBuffer.getInstance()
						.getAlElementsBuffer();
				for (HashMap mpElement : alElements) {
					if (mpElement.containsKey("class")
							&& mpElement.get("class")
									.equals("SlingshotElement")) {
						mpElement.put("kick", EditorBuffer.getInstance()
								.getSlingshotSpeed());
					}
				}
				...
				EditorBuffer.getInstance().putMpLaunchBufferElement("velocity",
						alLauncherSpeed);
				System.out.println(new JSONObject(EditorBuffer.getInstance()
						.getMpNewBoardBuffer()));
				System.out.println(new JSONObject(EditorBuffer.getInstance()
						.getMpLaunchBuffer()));
				try {
					FileHandle newBoard = Gdx.files.local("local/tables/"
							+ EditorBuffer.getInstance().getLevelname()
							+ ".json");
					System.out.println(newBoard.path());
					newBoard.file().createNewFile();
					FileHandle templateBoard = Gdx.files
							.internal("data/tables/new_board.json");
