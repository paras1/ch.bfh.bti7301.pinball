...
public class Physic {
	 ...
	 */
	public static Body createLine(World world, float x, float y,
			Vector2 direction) {
	...
	/**
	 * creates a wall with the given parameters
	 * 
	 * @param world
	 * @param xmin
	 * @param ymin
	 ...
	 * @return
	 */
	public static Body createWall(World world, float xmin, float ymin,
			float xmax, float ymax, float angle, float restitution) {
	...
	/**
	 * Creates the collisionListener for elements it detects all collisions and
	 * initiates the action e.g. count the score
	 * 
	 * @param world
	 */
		...
		world.setContactListener(new ContactListener() {
			...
			public void beginContact(Contact contact) {
				...
				if (fixtureB.getBody().getUserData() instanceof BumperElement) {
					...
					BumperElement bElement = (BumperElement) fixtureB.getBody()
							.getUserData();
					Gdx.app.log("Score: ", bElement.getScore() + "");
					GameState.getInstance().addScore(bElement.getScore());
				...
				if (fixtureA.getBody().getUserData() instanceof BumperElement) {
					...
					BumperElement bElement = (BumperElement) fixtureA.getBody()
							.getUserData();
					Gdx.app.log("Score: ", bElement.getScore() + "");
					GameState.getInstance().addScore(bElement.getScore());
				...
				if (fixtureB.getBody().getUserData() instanceof SlingshotElement) {
					...
					SlingshotElement slingshotElement = (SlingshotElement) fixtureB
							.getBody().getUserData();
				...
				if (fixtureA.getBody().getUserData() instanceof SlingshotElement) {
					Gdx.app.log("Slingshot touched: ", "");
					SlingshotElement slingshotElement = (SlingshotElement) fixtureA
							.getBody().getUserData();
